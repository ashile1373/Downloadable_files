...
public class CorrectionPlanRequirementsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_correction_plan_requirements);
        ...
        correctionSelectedPlan = dbHandler.confirmPlanSelectRow(planId, planModal.getUserID());
        selectedPlan = dbHandler.planSelectRow(planId);

        ownerSuggestionOne.setText(correctionSelectedPlan.getConfirmPlanOwnerSuggestionOne());
        if (!correctionSelectedPlan.getConfirmPlanOwnerSuggestionTwo().isEmpty()) {
            ownerSuggestionTwoLayout.setVisibility(View.VISIBLE);
            ownerSuggestionTwo.setText(correctionSelectedPlan.getConfirmPlanOwnerSuggestionTwo());
            addSuggestion.hide();
        }
        if (!correctionSelectedPlan.getConfirmPlanEducationNeeded().isEmpty()) {
            educationNeeded.setText(dbHandler.readHouseholdsFromID(correctionSelectedPlan.getConfirmPlanEducationNeeded()));
            if (correctionSelectedPlan.getConfirmPlanEducationNeeded().equals("1")) {
                addEducation.show();
                educationOneLayout.setVisibility(View.VISIBLE);
                educationOne.setText(correctionSelectedPlan.getConfirmPlanEducationOne());
                if (!correctionSelectedPlan.getConfirmPlanEducationTwo().isEmpty()) {
                    educationTwoLayout.setVisibility(View.VISIBLE);
                    educationTwo.setText(correctionSelectedPlan.getConfirmPlanEducationTwo());
                    addEducation.hide();
                }
            }
        }
        if (!correctionSelectedPlan.getConfirmPlanMarketingLevel().isEmpty())
            marketingLevel.setText(dbHandler.readPlanMarketingLevelNameFromID(correctionSelectedPlan.getConfirmPlanMarketingLevel()));
        finalEvaluation.setText(correctionSelectedPlan.getConfirmPlanFinalEvaluation());
        planStrength.setText(correctionSelectedPlan.getConfirmPlanStrength());
        planWeakness.setText(correctionSelectedPlan.getConfirmPlanWeakness());

        if (correctionSelectedPlan.getConfirmPlanSupplyValueChain() == 1)
            supply.setChecked(true);
        if (correctionSelectedPlan.getConfirmPlanDesignValueChain() == 1)
            designValueChain.setChecked(true);
        if (correctionSelectedPlan.getConfirmPlanProductionValueChain() == 1)
            productionValueChain.setChecked(true);
        if (correctionSelectedPlan.getConfirmPlanProcessingValueChain() == 1)
            processingValueChain.setChecked(true);
        if (correctionSelectedPlan.getConfirmPlanMarketingValueChain() == 1)
            marketingValueChain.setChecked(true);
        if (correctionSelectedPlan.getConfirmPlanOthersValueChain() == 1)
            othersValueChain.setChecked(true);

        if (correctionSelectedPlan.getConfirmNotUpToDateDataChallenge() == 1)
            notUpToDateDataChallenge.setChecked(true);
        if (correctionSelectedPlan.getConfirmHumanResourcesChallenge() == 1)
            humanResourcesChallenge.setChecked(true);
        if (correctionSelectedPlan.getConfirmCulturalProblemsChallenge() == 1)
            culturalProblemsChallenge.setChecked(true);
        if (correctionSelectedPlan.getConfirmLegalIssuesChallenge() == 1)
            legalIssuesChallenge.setChecked(true);
        if (correctionSelectedPlan.getConfirmRuleChangingChallenge() == 1)
            ruleChangingChallenge.setChecked(true);
        if (correctionSelectedPlan.getConfirmMarketingChallenge() == 1)
            marketingChallenge.setChecked(true);
        if (correctionSelectedPlan.getConfirmTariffChallenge() == 1)
            tariffChallenge.setChecked(true);
        if (correctionSelectedPlan.getConfirmImportSimilarProductChallenge() == 1)
            importSimilarProductChallenge.setChecked(true);
        if (correctionSelectedPlan.getConfirmFinancialChallenge() == 1)
            financialChallenge.setChecked(true);
        if (correctionSelectedPlan.getConfirmRAndDChallenge() == 1)
            rAndDChallenge.setChecked(true);
        if (correctionSelectedPlan.getConfirmSupplyRawMaterialsChallenge() == 1)
            supplyRawMaterialsChallenge.setChecked(true);
        if (correctionSelectedPlan.getConfirmInfrastructureChallenge() == 1)
            infrastructureChallenge.setChecked(true);
        if (correctionSelectedPlan.getConfirmOtherChallenge() == 1)
        ...
        cancelRequest.setOnClickListener(v -> {
            Intent intent = new Intent(CorrectionPlanRequirementsActivity.this, PendingToSuperVisionActivity.class);
        ...
        nextStep.setOnClickListener(v -> {
            if (checkErrors())
                ...
                correctionSelectedPlan.setConfirmPlanOwnerSuggestionOne(ownerSuggestionOne.getText().toString());
                correctionSelectedPlan.setConfirmPlanOwnerSuggestionTwo(ownerSuggestionTwo.getText().toString());
                correctionSelectedPlan.setConfirmPlanEducationNeeded(dbHandler.readHouseholdsID(educationNeeded.getText().toString()));
                if (dbHandler.readHouseholdsID(educationNeeded.getText().toString()).equals("1")) {
                    correctionSelectedPlan.setConfirmPlanEducationOne(educationOne.getText().toString());
                    correctionSelectedPlan.setConfirmPlanEducationTwo(educationTwo.getText().toString());
                } else {
                    correctionSelectedPlan.setConfirmPlanEducationOne("");
                    correctionSelectedPlan.setConfirmPlanEducationTwo("");
                }
                correctionSelectedPlan.setConfirmPlanMarketingLevel(dbHandler.readPlanMarketingLevelID(marketingLevel.getText().toString()));
                correctionSelectedPlan.setConfirmPlanFinalEvaluation(finalEvaluation.getText().toString());
                correctionSelectedPlan.setConfirmPlanStrength(planStrength.getText().toString());
                correctionSelectedPlan.setConfirmPlanWeakness(planWeakness.getText().toString());

                if (supply.isChecked())
                    correctionSelectedPlan.setConfirmPlanSupplyValueChain(1);
                else correctionSelectedPlan.setConfirmPlanSupplyValueChain(0);
                if (designValueChain.isChecked())
                    correctionSelectedPlan.setConfirmPlanDesignValueChain(1);
                else correctionSelectedPlan.setConfirmPlanDesignValueChain(0);
                if (productionValueChain.isChecked())
                    correctionSelectedPlan.setConfirmPlanProductionValueChain(1);
                else correctionSelectedPlan.setConfirmPlanProductionValueChain(0);
                if (processingValueChain.isChecked())
                    correctionSelectedPlan.setConfirmPlanProcessingValueChain(1);
                else correctionSelectedPlan.setConfirmPlanProcessingValueChain(0);
                if (marketingValueChain.isChecked())
                    correctionSelectedPlan.setConfirmPlanMarketingValueChain(1);
                else correctionSelectedPlan.setConfirmPlanMarketingValueChain(0);
                if (othersValueChain.isChecked())
                    correctionSelectedPlan.setConfirmPlanOthersValueChain(1);
                else correctionSelectedPlan.setConfirmPlanOthersValueChain(0);

                if (notUpToDateDataChallenge.isChecked())
                    correctionSelectedPlan.setConfirmNotUpToDateDataChallenge(1);
                else correctionSelectedPlan.setConfirmNotUpToDateDataChallenge(0);
                if (humanResourcesChallenge.isChecked())
                    correctionSelectedPlan.setConfirmHumanResourcesChallenge(1);
                else correctionSelectedPlan.setConfirmHumanResourcesChallenge(0);
                if (culturalProblemsChallenge.isChecked())
                    correctionSelectedPlan.setConfirmCulturalProblemsChallenge(1);
                else correctionSelectedPlan.setConfirmCulturalProblemsChallenge(0);
                if (legalIssuesChallenge.isChecked())
                    correctionSelectedPlan.setConfirmLegalIssuesChallenge(1);
                else correctionSelectedPlan.setConfirmLegalIssuesChallenge(0);
                if (ruleChangingChallenge.isChecked())
                    correctionSelectedPlan.setConfirmRuleChangingChallenge(1);
                else correctionSelectedPlan.setConfirmRuleChangingChallenge(0);
                if (marketingChallenge.isChecked())
                    correctionSelectedPlan.setConfirmMarketingChallenge(1);
                else correctionSelectedPlan.setConfirmMarketingChallenge(0);
                if (tariffChallenge.isChecked())
                    correctionSelectedPlan.setConfirmTariffChallenge(1);
                else correctionSelectedPlan.setConfirmTariffChallenge(0);
                if (importSimilarProductChallenge.isChecked())
                    correctionSelectedPlan.setConfirmImportSimilarProductChallenge(1);
                else correctionSelectedPlan.setConfirmImportSimilarProductChallenge(0);
                if (financialChallenge.isChecked())
                    correctionSelectedPlan.setConfirmFinancialChallenge(1);
                else correctionSelectedPlan.setConfirmFinancialChallenge(0);
                if (rAndDChallenge.isChecked())
                    correctionSelectedPlan.setConfirmRAndDChallenge(1);
                else correctionSelectedPlan.setConfirmRAndDChallenge(0);
                if (supplyRawMaterialsChallenge.isChecked())
                    correctionSelectedPlan.setConfirmSupplyRawMaterialsChallenge(1);
                else correctionSelectedPlan.setConfirmSupplyRawMaterialsChallenge(0);
                if (infrastructureChallenge.isChecked())
                    correctionSelectedPlan.setConfirmInfrastructureChallenge(1);
                else correctionSelectedPlan.setConfirmInfrastructureChallenge(0);
                if (otherChallenge.isChecked())
                    correctionSelectedPlan.setConfirmOtherChallenge(1);
                else correctionSelectedPlan.setConfirmOtherChallenge(0);
                ...
                Intent intent = new Intent(CorrectionPlanRequirementsActivity.this, DoneActivity.class);
    ...
    public void onBackPressed() {
        ...
        Intent intent = new Intent(CorrectionPlanRequirementsActivity.this, CorrectionPlanFacilityActivity.class);
        intent.putExtra("planId", planId);
        startActivity(intent);
        overridePendingTransition(0, 0);
        finish();
...
//        titleText.setText(title);
        ...
        Toast toast = new Toast(CorrectionPlanRequirementsActivity.this);
